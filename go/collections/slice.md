...
# = Slice =
Можно сказать, что __срез__ - это __массив__ переменной длинны
__Срез__ - структура данных, которая содержит _несколько_ значений:
    * Указатель на [[array.md|массив]]
	* __Длинну__ (__Lenght__) среза - кол-во элементов которые он может содержать в данный момент
	* __Ёмкость__ (__Capacity__) среза - это кол-во элементов, которые могут быть сохранены в базовом массиве, прежде чем размер среза должен быть изменен и создан новый массив
