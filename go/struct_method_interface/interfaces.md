...
# = Interfaces =
__Интерфейс__ (__И__) - описывает набор __М__ без указания их реализации. Если __тип__ реализует все методы, определенные интерфейсом, то значение этого __типа__ можно использовать везде где разрешен данный __И__
Пример:
```
################
###product.go###
