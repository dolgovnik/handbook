...
_Сравнимая_ __С__ может использоваться в качестве _ключа_ в __К__
...
__С__ без полей - это _пустая_ __С__ - `struct{}` - может применяться в __К__ вместо `bool` для создания `set` - так экономится место.
