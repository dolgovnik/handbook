...
`someTotal += <- channel` - полученеи значения из __КН__ _channel_. __!!!__ Это __блокирующая__ операция - выполнение не будет продолжаться до тех пор, пока не будет получено значение
...
* Если вторая __ГР__ отправляет значение, она будет заблокирована до тех пор, пока канал не будет очищен, что приведе к созданию очереди из __ГР__ ожидающих получения значений.
