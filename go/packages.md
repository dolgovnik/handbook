...
`go mod init <package_name>` создает файл модуля (`go.mod`), он упрощает установку опубликованных __П__, позволяет испльзовать `go run .` - все само компилируется.
```
############
###go.mod###
############
...
import (
    "fmt"
	"packages/store"    // <-- <имя модуля>/<имя пакета>
)
